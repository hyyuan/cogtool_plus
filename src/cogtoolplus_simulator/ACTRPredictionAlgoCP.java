...
import edu.cmu.cs.hcii.cogtool.model.ACTRTraceParser;
...
import edu.cmu.cs.hcii.cogtool.model.HomeAction;
...
import edu.cmu.cs.hcii.cogtool.model.TransitionDelayScriptStep;
...
import edu.cmu.cs.hcii.cogtool.model.AScriptStep.ScriptStepVisitor;
import edu.cmu.cs.hcii.cogtool.model.IPredictionAlgo.AAnalysisInput;
import edu.cmu.cs.hcii.cogtool.model.IPredictionAlgo.ComputationException;
import edu.cmu.cs.hcii.cogtool.model.IPredictionAlgo.IAnalysisInput;
import edu.cmu.cs.hcii.cogtool.model.IPredictionAlgo.IAnalysisOutput;
...
public class ACTRPredictionAlgoCP extends APredictionAlgo
{
    ...
    public static final String USES_OBSOLETE_WAITS =
        "ACTRPredictionAlgoCP.usesObsoleteWaits";
    public static final String TRACE_VERSION_ATTR =
        "ACTRPredictionAlgoCP.traceVersion";
    ...
    private static double getActrTimeoutInSeconds() {
        return CogToolPrefCP.ACTR_TIMEOUT.getInt() / 1000.0;
    }
    
    public static final int edu_cmu_cs_hcii_cogtool_model_ACTRPredictionAlgo_version = 0;

    private static ObjectSaver.IDataSaver<ACTRPredictionAlgoCP> SAVER =
        new ObjectSaver.ADataSaver<ACTRPredictionAlgoCP>() {
            @Override
            ...
            @Override
            public void saveData(ACTRPredictionAlgoCP v, ObjectSaver saver)
    ...
    {
        ObjectSaver.registerSaver(ACTRPredictionAlgoCP.class.getName(), SAVER);
    }

    private static ObjectLoader.IObjectLoader<ACTRPredictionAlgoCP> LOADER =
        new ObjectLoader.AObjectLoader<ACTRPredictionAlgoCP>();

    // This object will always be ignored when used during loading
    // of scripts and results (see TaskApplication version 0 loading)
    private static final ACTRPredictionAlgoCP StandaloneAlgo_ONLY =
        new ACTRPredictionAlgoCP();
    
    private static ObjectLoader.IObjectLoader<ACTRPredictionAlgoCP> StandaloneAlgo_LOADER =
        new ObjectLoader.AObjectLoader<ACTRPredictionAlgoCP>() {
            @Override
            public ACTRPredictionAlgoCP createObject()
            {
    ...
    {
        ObjectLoader.registerLoader(ACTRPredictionAlgoCP.class.getName(),
    ...
    protected ACTRPredictionAlgoCP() { }
    ...
    protected class ACTRAnalysisOutput implements IAnalysisOutput
    {
        ...
        protected List<String> traceLines = new ArrayList<String>();
        ...
        public APredictionResult completeWork()
        {
            ...
            if (lineCount > 0) {
                try {
                    //System.out.println("ACTRPredictionAlgoCP is 4");
...
//                  resultSteps = TraceReducer.simplifyTrace(resultSteps);
                    ...
                    if (! equalTimes(taskTime, (double)OLD_TIMEOUT_VALUE)) {
                        //System.out.println("ACTRPredictionAlgoCP is 5");

                    	result = new TimePredictionResult("ACT-R Time: " + taskTime,
                                                          script,
                                                          ACTRPredictionAlgoCP.this,
                                                          traceLines,
                                                          errorLines,
                                                          resultSteps,
                                                          taskTime);
                        ...
                        //System.out.println("ACTRPredictionAlgoCP is 6");
                    ...
                    //System.out.println("ACTRPredictionAlgoCP is 7");
                ...
                //System.out.println("ACTRPredictionAlgoCP is 8");
    ...
    protected class ACTRAnalysisInput extends AAnalysisInput
    {
        ...
        {
            //System.out.println("ACTRPredictionAlgoCP is 3");
    ...
    public IAnalysisInput prepareComputation(Script script)
    {
        ...
        //System.out.println("ACTRPredictionAlgoCP path is " + path);
        File actrFile = null;
        if (path != null) {
            ...
            try {
                //System.out.println("ACTRPredictionAlgoCP is 1");
                ...
                //System.out.println("ACTRPredictionAlgoCP is 2");
    ...
    {
        out.println("(terpri)");
        if (CogToolPrefCP.isTraceEmitted()) {
        ...
        if (CogToolPrefCP.RESEARCH.getBoolean()) {
            out.println("(setq *cogtool-debug* " + 
                        CogToolPrefCP.ACTR_DEBUG_LEVEL.getInt() +
                        ")");
        }
        boolean useACTRDefaults = 
                (!CogToolPrefCP.RESEARCH.getBoolean() || !CogToolPrefCP.ACTR_ALTERNATIVE_PARAMETERS.getBoolean());
        out.format(Locale.US,
                   ("(setq *overridden-global-parameters* `(" +
                    ":visual-attention-latency %.3f :motor-initiation-time %.3f :peck-fitts-coeff %.3f :dat %.3f  :min-fitts-time %.3f" +
                    ",@*overridden-global-parameters*))\n"),
                    (useACTRDefaults ? CogToolPrefCP.VISUAL_ATTENTION.getIntDefault() : CogToolPrefCP.VISUAL_ATTENTION.getInt()) / 1000.0,
                    (useACTRDefaults ? CogToolPrefCP.MOTOR_INITIATION.getIntDefault() : CogToolPrefCP.MOTOR_INITIATION.getInt()) / 1000.0,
                    (useACTRDefaults ? CogToolPrefCP.PECK_FITTS_COEFF.getIntDefault() : CogToolPrefCP.PECK_FITTS_COEFF.getInt()) / 1000.0,                   
                    (useACTRDefaults ? CogToolPrefCP.ACTR_DAT.getIntDefault() : CogToolPrefCP.ACTR_DAT.getInt()) / 1000.0,
                    (useACTRDefaults ? CogToolPrefCP.MIN_FITTS.getIntDefault() : CogToolPrefCP.MIN_FITTS.getInt()) /1000.0); 
                    //This corresponds to the 'min-fitts-time %.3f' used in lisp ACT-R (this parameter can be found in cogtool-actr.lisp)
        ...
        throws IOException
    {
        backButtonLabels = new HashSet<String>(
                Arrays.asList(CogToolPrefCP.CTE_DEFAULT_BACK_LABEL.getString().split(BACK_BUTTON_SEPARATOR)));
        PrintWriter w = null;
        try {
            w = new PrintWriter(
                   new BufferedWriter(
                      new OutputStreamWriter(
                          ...
                          "US-ASCII")));
            if (CogToolPrefCP.SYSWVO.getBoolean()) {
            ...
            if (CogToolPrefCP.USE_EMMA.getBoolean()) {
                w.println(";; EMMA\n");
        ...
        throws IOException
    {
        ...
        File logFile = new File(CogToolPrefCP.LOG_DIRECTORY.getString(), CTE_LOG_FILE);
        ...
        if (! CogToolPrefCP.CTE_SUPPRESS_NOISE.getBoolean()) {
            lispVars.put("*cogtool-random-seed*", "nil");
        }
        int backButtonSemantics = CogToolPrefCP.CTE_BACK_BUTTON_SEMANTICS.getInt();
        ...
        String logDir = CogToolPrefCP.LOG_DIRECTORY.getString();
        if (!logDir.endsWith("/")) {
            logDir += "/";
        }
    ...
    private static void outputFrame(Frame f, PrintWriter out)
    {
        ...
        for (IWidget widget : f.getWidgets()) {
            if (widget.getWidgetType() == WidgetType.Noninteractive &&
                    ...
                    CogToolPrefCP.CTE_SUPPRESS_NONINTERACTIVE.getBoolean()) {
                continue;
            }
